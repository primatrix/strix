"""Ablation: skip fp32→bf16 cast (write-back DMA unchanged).

Isolates the cost of the astype conversion. Last tile compute and
output DMA are unchanged; output content will be garbage.
"""
...
def _double_buffer_expert_kernel(
    ...
    *,
    ...
    # -- Write-back: skip cast, DMA b_y_out_vmem (garbage) → output_hbm --
    pltpu.make_async_copy(
...
if __name__ == "__main__":
    ...
    print(f"bt={bt}, bf={bf_arg}, result_shape={result.shape}")
    print("PASS (no correctness check — ablation variant)")

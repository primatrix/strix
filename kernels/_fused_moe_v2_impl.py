...
Ported from sglang-jax/python/sgl_jax/srt/kernels/fused_moe/v2/kernel.py
...
def _fused_ep_moe_kernel(
    ...
    # Dequant scratch (single-buf, None when not quantized)
...
def fused_ep_moe_v2(
    ...
    *,
    ...
    scratch_shapes = (
        ...
        pltpu.VMEM((bts, bf), jnp.float32),
        pltpu.VMEM((bts, bf), jnp.float32),
        pltpu.VMEM((bts, t_packing, h_per_t), t_dtype),
        pltpu.VMEM((bts, t_packing, h_per_t), jnp.float32),
        pltpu.VMEM((bts, t_packing, h_per_t), t_dtype),
        (None if w1_shared is None else
            pltpu.VMEM((2, 2, bt, t_packing, h_per_t), t_dtype)),
        (None if w1_shared is None else
            pltpu.VMEM((2, t_packing, h_per_t, bse), w1.dtype)),
        (None if w3_shared is None else
            pltpu.VMEM((2, t_packing, h_per_t, bse), w3.dtype)),
        (None if w2_shared is None else
            pltpu.VMEM((2, t_packing, bse, h_per_t), w2.dtype)),
        (None if w1_shared is None else
            pltpu.VMEM((2, bt, hidden_size), jnp.float32)),
        ...
        pl.pallas_call(
            ...
            grid_spec=pltpu.PrefetchScalarGridSpec(
                ...
                in_specs=[
                    ...
                    hbm_spec,
                    hbm_spec,
                    None if w1_shared is None else hbm_spec,
                    None if w3_shared is None else hbm_spec,
                    None if w2_shared is None else hbm_spec,
                    None if not needs_jax_allreduce else hbm_spec,
                    None if not needs_jax_allreduce else hbm_spec,
                    None if not needs_jax_allreduce else hbm_spec,
                ],
    ...
    @jax.shard_map(
        ...
        in_specs=(
            ...
            P((dp_axis_name, tp_axis_name)),
            P((dp_axis_name, tp_axis_name)),
            None if w1_scale is None else P((dp_axis_name, tp_axis_name)),
            None if w2_scale is None else P((dp_axis_name, tp_axis_name)),
            None if w3_scale is None else P((dp_axis_name, tp_axis_name)),
            P((dp_axis_name, tp_axis_name)),
            P((dp_axis_name, tp_axis_name)),
            P(),
            P(),
            P(),
            None if w1_shared is None else P(),
            None if w3_shared is None else P(),
            None if w2_shared is None else P(),
        ),

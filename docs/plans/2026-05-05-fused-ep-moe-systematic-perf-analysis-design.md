...
## 2.5 各阶段理论 Roofline 分析

本节对每个阶段应用 Roofline 模型，推导理论耗时、瓶颈类型和时间分布。

### 2.5.1 Roofline 框架

每个阶段的理论耗时由三类资源的最大值决定:

$$T_{\text{stage}} = \max\left(\underbrace{\frac{\text{FLOPs}}{F_{MXU}}}_{T_{\text{compute}}}, \underbrace{\frac{\text{Bytes}_{HBM}}{BW_{HBM}}}_{T_{\text{HBM}}}, \underbrace{\frac{\text{Bytes}_{ICI}}{BW_{ICI}}}_{T_{\text{ICI}}}\right)$$

其中 Arithmetic Intensity $\text{AI} = \text{FLOPs} / \text{Bytes}_{HBM}$，与 Ridge Point $R = F_{MXU} / BW_{HBM}$ 比较:

- $\text{AI} < R$: **HBM-bound**, $T_{\text{stage}} = \text{Bytes}_{HBM} / BW_{HBM}$
- $\text{AI} \geq R$: **Compute-bound**, $T_{\text{stage}} = \text{FLOPs} / F_{MXU}$

对于无 MXU 计算的阶段 (S1, S2a, S2c, S3)，耗时由 HBM DMA、ICI 通信或 VPU 延迟决定。

### 2.5.2 逐阶段理论耗时公式

#### Stage 1: Routing + AllReduce Metadata

无 MXU 计算，耗时由 ICI allgather 和 VPU routing 逻辑决定:

$$T_{S1} = \underbrace{\lceil \log_2 P_{EP} \rceil \times (t_{\text{barrier}} + t_{\text{ICI}})}_{\text{allgather 延迟}} + \underbrace{\frac{P_{EP} \times N_E \times 4}{BW_{ICI}}}_{\text{allgather 数据传输}} + \underbrace{\frac{bt \times k \times N_E}{V_{VPU}}}_{\text{routing mask 生成}}$$

| 资源 | 数据量 / 计算量 | 耗时公式 |
|------|---------------|---------|
| MXU | 0 | 0 |
| HBM | $8 \times bt \times k$ bytes | $\ll 1 \mu s$ |
| ICI | $P_{EP} \times N_E \times 4$ bytes | $\sim 5\text{-}10 \mu s$ |
| VPU | $O(bt \times k \times N_E)$ ops | $\sim 5\text{-}10 \mu s$ |
| **Barrier** | $\lceil \log_2 P_{EP} \rceil + 1$ 次 | $\sim 5\text{-}15 \mu s$ |

#### Stage 2a: AllToAll Scatter (Dispatch)

纯数据搬运，无计算:

$$T_{S2a} = \max\left(\frac{bt \times k \times H \times B_a}{BW_{HBM}}, \frac{bt \times k \times (1 - E_L/N_E) \times H \times B_a}{BW_{ICI}}, bt \times k \times t_{\text{DMA}}\right)$$

| 资源 | 数据量 | 耗时公式 |
|------|-------|---------|
| MXU | 0 | 0 |
| HBM 写入 | $bt \times k \times H \times B_a$ | $\text{Bytes} / BW_{HBM}$ |
| ICI | $bt \times k \times (1 - E_L/N_E) \times H \times B_a$ | $\text{Bytes} / BW_{ICI}$ |
| DMA setup | $bt \times k$ 次 | $bt \times k \times t_{\text{DMA}}$ |

#### Stage 2b: Expert FFN Compute (核心瓶颈)

这是唯一适用经典 Roofline 分析的阶段:

$$T_{S2b}^{\text{HBM}} = \frac{E_L \times 3 \times H \times I \times B_w}{BW_{HBM}}$$

$$T_{S2b}^{\text{compute}} = \frac{E_L \times 6 \times bts \times H \times I}{F_{MXU}}$$

$$T_{S2b} = \max(T_{S2b}^{\text{HBM}}, T_{S2b}^{\text{compute}})$$

**Roofline 判定**:

$$\frac{T_{S2b}^{\text{HBM}}}{T_{S2b}^{\text{compute}}} = \frac{B_w \times F_{MXU}}{2 \times bts \times BW_{HBM}} = \frac{R}{bts}$$

- 当 $bts < R$ (即 $bts < 313$, v7x BF16): **HBM-bound**, $T_{S2b} = T_{S2b}^{\text{HBM}}$
- 当 $bts \geq R$: **Compute-bound**, $T_{S2b} = T_{S2b}^{\text{compute}}$
...
与 S2b expert loop pipeline 化，可部分或完全被覆盖:

$$T_{S2c}^{\text{total}} = \frac{E_L \times \bar{n}_e \times H \times B_a}{BW_{ICI}} \times (1 - E_L/N_E) + E_L \times P_{EP} \times t_{\text{DMA}}$$
...
|------|-------|---------|
| MXU | 0 | 0 |
| HBM 写入 | $T_L \times k \times H \times B_a$ | $\text{Bytes} / BW_{HBM}$ |
| ICI | $T_L \times k \times (1 - E_L/N_E) \times H \times B_a$ | $\text{Bytes} / BW_{ICI}$ |
| DMA setup | $E_L \times P_{EP}$ 次 | $E_L \times P_{EP} \times t_{\text{DMA}}$ |

> Decode 下 $T_{\text{expert}} \gg T_{\text{gather}}^{\text{per expert}}$, gather 完全被覆盖: $T_{S2c}^{\text{visible}} \approx 0$。
...
$$T_{\text{critical}} = T_{S1} + T_{S2a} + T_{S2b} + T_{S2c}^{\text{visible}} + T_{S3}$$

$$T_{\text{overlap}} = T_{S4} \quad \text{(与 critical path 并行)}$$

$$T_{\text{total}} = \max(T_{\text{critical}}, T_{\text{overlap}})$$

> 通常 $T_{\text{critical}} \gg T_{\text{overlap}}$, 即 $T_{\text{total}} \approx T_{\text{critical}}$。

#### 各阶段占比公式

$$f_{Si} = \frac{T_{Si}}{T_{\text{critical}}} \times 100\%$$

#### Ling 2.6 Decode 实例化 ($N_E = 256, H = 8192, I = 2048, k = 8, T = 256, P_{EP} = 4$)
...
$$T_{\text{critical}} = 15 + 40 + 1{,}665 + 0 + 10 \approx 1{,}730 \; \mu s$$

$$T_{S4} = 138 \; \mu s \ll T_{\text{critical}} \implies T_{\text{total}} \approx 1{,}730 \; \mu s$$

...
> 以下文档基于 [How to Scale Your Model](https://jax-ml.github.io/scaling-book/) 系列整理，提供 kernel 开发所需的性能分析背景知识。

| 文档 | 说明 |
|------|------|
| [Roofline 分析深度指南](./roofline-analysis) | 算术强度公式、临界 batch size、tiling 强度、多芯片通信 Roofline |
| [TPU 硬件规格参考](./tpu-specs) | 跨代次 TPU 计算/内存/互联规格表、VPU 详细规格、GPU 对比 |
| [Sharding 与集合通信参考](./sharding-collectives) | Sharding 标记法、AllGather/ReduceScatter/AllReduce 开销、并行策略 |
| [Transformer 算子性能参考](./transformer-ops-reference) | 各组件 FLOPs 计算、注意力算术强度、Flash Attention、KV Cache、推理优化 |
| [Profiling 深度指南](./profiling-guide) | 读懂 HLO 和 XLA Op、Tiling/Layout 标记、计算预期耗时、常见性能问题 |

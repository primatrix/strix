...
- `p/0-blocker`: 阻塞级。需在当日日报中置顶显示。
- `p/1-urgent`: 紧急。需在当日日报中置顶显示。
- `p/2-high`: 高优先级。
- `p/3-normal`: 普通。
...
| **`status/design-pending`** | **Size L 专属** | 方案设计/评审中。需产出 PRD/RFC 或设计文档。 |
...
- `size/L` (Large): 大规模任务。需遵循完整 SOP，包括方案评审及 E2E 测试。
...
- `beaver/missing-approval`: 缺少设计审批。`design-pending → ready-to-develop` 转换需要至少一条 `/approve` 评论，但未检测到有效的审批评论。
...
- `beaver/wontfix`: 不予修复。标记为不打算处理的 Issue，Beaver 将跳过此类 Issue 的停滞检测和超期提醒。
...
| :--- | :--- | :--- |
| **size/L** (大) | **标准 SOP (全生命周期)** | 关注设计对齐、多阶段 Review 延迟、里程碑风险。 |
...
```mermaid
graph TD
    A[status/triage] --> B{Beaver 识别规模}

    B -->|size/L| D[status/design-pending]
    D -->|需要 /approve 评论| E[status/ready-to-develop]
    E --> F[status/in-progress]

    B -->|size/S| F

    F --> G[status/review-needed]
    F --> BLK[status/blocked]
    BLK --> F
    G --> H[status/done]
    G --> QA[status/qa-testing]
    QA --> H
...
| **`status/design-pending`** | **Size L** | 挂载标签 | **Gatekeeper**：强检 PRD/RFC 及方案文档。提醒架构师评审。停留 >3d 周报预警。 |
| **`status/ready-to-develop`** | **Size L** | 状态变更 | **晨间提醒**：加入开发者今日待办清单。 |
| **`status/in-progress`** | 通用 | 状态变更 | **日报**：列入活跃清单。无提交停留 >5d 自动询问。（标记 `beaver/wontfix` 的 Issue 跳过停滞检测） |
| **`status/review-needed`** | 通用 | 状态变更/PR | **实时告警**：私信 Reviewer。校验 CI。 |
| **`status/blocked`** | 通用 | 状态变更 | **实时告警**：频道红字预警。LLM 提取根因至日报。 |
| **`status/qa-testing`** | **Size L 建议** | 状态变更 | **Gatekeeper**：校验测试覆盖率与回归结果。停留过久时周报预警。 |
...
| **已逾期 (Overdue)** | 当前时间 > DDL | 识别为进度脱控 | **实时告警**：向管理频道发送"进度报警"。**日报**：将任务标记为红色置顶。（标记 `beaver/wontfix` 的 Issue 跳过超期提醒） |
...
- ❌ `status/triage` ➔ `status/in-progress` (针对 `size/L`): **禁止**。大型任务必须经过设计阶段 (`status/design-pending`)。
...
### 6.4 设计审批门禁 (Design Approval Gate)

- **审批要求**：`status/design-pending` ➔ `status/ready-to-develop` 转换前，Issue 评论区必须包含至少一条 `/approve` 命令（不区分大小写，必须出现在行首）。
- **触发行为**：若未检测到 `/approve` 评论，Beaver 将挂载 `beaver/missing-approval` 标签并发表评论提醒，但不会回滚 `status/ready-to-develop` 标签。用户在 Issue 下评论 `/approve` 后，下次 `status/ready-to-develop` 标签变更时 Beaver 将放行并自动清理 `beaver/missing-approval`。
- **审批人范围**：不限制评论者身份，任何人的 `/approve` 评论均视为有效审批。

...
- **定位**：团队每日进度的"快照"，侧重**信息同步与阻塞暴露**。
- **触发时间**：工作日 18:00 (本地时间)。
- **核心内容**：
  - **今日成果**：汇总当日已合并的 PR、已关闭的 Issue 及状态流转变更。
  - **项目进展**：按 Project 维度呈现各看板列的任务增减变化。
  - **阻塞与风险**：高亮当日新增的阻塞项、超过 24h 未响应的 PR Review 请求。
  - **明日预期**：列出当前 `status/in-progress` 的任务及其预期产出。

#### D. 智能周报 (Weekly Deep Analysis)
...
#### E. 流程守门员 (Workflow Gatekeeper) - **核心增强**
...
#### F. 任务拆解、审计与跨任务依赖 (Decomposition, Audit & Dependencies) - **核心增强**
...
- **测试结论验证 (Test Evidence Verification)**：
  ...
  #### H. 迭代复盘自动化 (Sprint Retrospective) - **新增**

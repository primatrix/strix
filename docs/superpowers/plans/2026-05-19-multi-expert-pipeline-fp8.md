# Multi-Expert Pipeline FP8 Block Quantization Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Create `kernels/multi_expert_pipeline_fp8.py` — an FP8 e4m3fn variant of the multi-expert double-buffer FFN pipeline kernel with VMEM dequant, per-block scales, and correctness validation.

**Architecture:** New file mirrors the structure of `kernels/multi_expert_pipeline.py` (bf16 version) but replaces bf16 weight buffers with fp8, adds per-block f32 scale double-buffers + bf16 dequant scratch, and inserts dequant loops (fp8→bf16) before each matmul. The DMA pipeline follows "Timing C" — next-tile prefetch fires after `wait_fetch_w2`. No changes to the existing bf16 kernel file.

**Tech Stack:** JAX, Pallas (TPU), `jax.experimental.pallas.tpu` for `make_async_copy`, `SemaphoreType.DMA`, `VMEM` scratch shapes. FP8 dtype: `jnp.float8_e4m3fn`.
...
|------|--------|---------------|
| `kernels/multi_expert_pipeline_fp8.py` | **Create** | FP8 kernel: config, Pallas kernel body, JIT entry, reference impl, kernel_fn, __main__ |
| `kernels/multi_expert_pipeline.py` | **Do not modify** | Existing bf16 kernel — unchanged |

---

### Task 1: Scaffold file with config, imports, reference impl, and helpers

This task creates the file with everything *except* the Pallas kernel and JIT entry point. The reference implementation and helpers are standalone pure-JAX code that can be tested immediately.

**Files:**
- Create: `kernels/multi_expert_pipeline_fp8.py`

- [ ] **Step 1: Create the file with module docstring, imports, config, and helpers**

Write `kernels/multi_expert_pipeline_fp8.py`:
...
Run: `python -c "from kernels.multi_expert_pipeline_fp8 import config, _ref_multi_expert_ffn_fp8; print('OK:', config['description'])"`

Expected: `OK: Multi-expert double-buffer FFN — FP8 e4m3 block quantization (VMEM dequant path, N experts sequential)`

- [ ] **Step 3: Commit scaffold**

```bash
git add kernels/multi_expert_pipeline_fp8.py
git commit -m "feat(fp8): scaffold multi_expert_pipeline_fp8 with config, reference impl, and helpers"
```

---

### Task 2: Pallas kernel body — DMA helpers and dequant functions

Add `_multi_expert_kernel_fp8()` — the Pallas kernel body. This task writes the DMA helper functions (start/wait for weights+scales, tokens, writeback) and the three dequant functions. The expert loop is added in Task 3.

**Files:**
- Modify: `kernels/multi_expert_pipeline_fp8.py`

- [ ] **Step 1: Add the kernel function signature and DMA helpers**

Append to `kernels/multi_expert_pipeline_fp8.py` (after `_ref_multi_expert_ffn_fp8`):
...
**Note**: Leave the function body here (no expert loop yet). The prologue and expert loop are added in Task 3 below this code.

- [ ] **Step 2: Commit DMA helpers and dequant**

```bash
git add kernels/multi_expert_pipeline_fp8.py
git commit -m "feat(fp8): add Pallas kernel DMA helpers and dequant functions"
```

---

### Task 3: Expert loop with Timing-C pipeline

Complete the `_multi_expert_kernel_fp8` body: the global prologue, the `expert_body` fori_loop with Timing-C prefetch scheduling, and the drain epilogue.

**Files:**
- Modify: `kernels/multi_expert_pipeline_fp8.py`

- [ ] **Step 1: Add the expert loop to the kernel body**

Append the following to the end of `_multi_expert_kernel_fp8` (inside the function, after `dequant_w2`):
...
- [ ] **Step 2: Commit expert loop**

```bash
git add kernels/multi_expert_pipeline_fp8.py
git commit -m "feat(fp8): add expert loop with Timing-C pipeline to FP8 kernel"
```

---

### Task 4: JIT entry point (`multi_expert_ffn_fp8`)

Wire up the Pallas kernel with scratch shapes, HBM specs, validation, and the public `@jax.jit` function.

**Files:**
- Modify: `kernels/multi_expert_pipeline_fp8.py`

- [ ] **Step 1: Add the JIT entry point**

Append to `kernels/multi_expert_pipeline_fp8.py` (after `_multi_expert_kernel_fp8`):
...
Run: `python -c "from kernels.multi_expert_pipeline_fp8 import multi_expert_ffn_fp8; print('OK')"`

Expected: `OK`

- [ ] **Step 3: Commit JIT entry**

```bash
git add kernels/multi_expert_pipeline_fp8.py
git commit -m "feat(fp8): add multi_expert_ffn_fp8 JIT entry point with scratch shapes and validation"
```

---

### Task 5: `kernel_fn()` factory and `__main__` correctness test

Add the sweep-integration factory and standalone correctness test that compares the Pallas kernel output against the pure-JAX reference.

**Files:**
- Modify: `kernels/multi_expert_pipeline_fp8.py`

- [ ] **Step 1: Add `kernel_fn` and `__main__`**

Append to `kernels/multi_expert_pipeline_fp8.py`:
...
- [ ] **Step 2: Verify the full file is importable**

Run: `python -c "from kernels.multi_expert_pipeline_fp8 import kernel_fn, config; print('kernel_fn:', kernel_fn); print('config:', config['weight_dtype'])"`

Expected:
```
kernel_fn: <function kernel_fn at 0x...>
config: float8_e4m3fn
```

- [ ] **Step 3: Commit kernel_fn and __main__**

```bash
git add kernels/multi_expert_pipeline_fp8.py
git commit -m "feat(fp8): add kernel_fn factory and __main__ correctness test"
```

---

### Task 6: Run correctness test on TPU

Execute the `__main__` correctness test on a TPU to verify the kernel output matches the reference implementation.
...
Run on TPU (via exec-remote or direct SSH):
```bash
python -m kernels.multi_expert_pipeline_fp8 4 256 6144 2048
```

Expected output pattern:
```
  expert 0: max_abs_err=X.XXXX, rel_err=0.XXXXXX
  expert 1: max_abs_err=X.XXXX, rel_err=0.XXXXXX
  expert 2: max_abs_err=X.XXXX, rel_err=0.XXXXXX
  expert 3: max_abs_err=X.XXXX, rel_err=0.XXXXXX
num_experts=4, bt=256, d=6144, f=2048, bf=256, qbk=256, worst_rel_err=0.XXXXXX
PASS
```

If `FAIL` — check:
1. Scale shapes mismatch → print shapes at start of `__main__`
2. Dequant indexing bug → compare dequant output slice against manual computation
3. DMA ordering → add `disable_*` flags like the v2 kernel for isolation testing

- [ ] **Step 2: Test with smaller config to verify edge cases**

```bash
python -m kernels.multi_expert_pipeline_fp8 2 256 2048 512
```

This tests a smaller hidden_size (2048) and intermediate_size (512) to verify the dequant sub-group counts work correctly with `n_sg=8, n_sg2=1`.

- [ ] **Step 3: Create a final commit if any fixes were needed**

If the test required bug fixes, commit them:
```bash
git add kernels/multi_expert_pipeline_fp8.py
git commit -m "fix(fp8): correct [describe what was fixed] in FP8 kernel"
```
